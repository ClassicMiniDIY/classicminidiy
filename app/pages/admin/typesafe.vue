...
<template>
  ...
  >
    ...
    <template v-else-if="data">
      ...
      <!-- One card at a time. A left-hand menu rather than a tab strip: eight
           labelled tabs overflow the shell's width and the scrollbar clips
           them. Under md the menu becomes a select. -->
      <div class="grid grid-cols-1 gap-6 md:grid-cols-[240px_minmax(0,1fr)]">
        <div class="md:hidden">
          <select v-model="tab" class="select select-bordered w-full" aria-label="Card">
            <option v-for="t in TABS" :key="t.key" :value="t.key">
              {{ t.label }}{{ tabGrade[t.key] ? ` — ${gradeLabel(tabGrade[t.key]!)}` : '' }}
            </option>
          </select>
        </div>
        <nav class="hidden md:block">
          <ul class="menu sticky top-4 w-full rounded-box border border-base-300 bg-base-100 p-2">
            <li v-for="t in TABS" :key="t.key">
              <button
                type="button"
                class="flex items-center gap-3"
                :class="{ 'menu-active': tab === t.key }"
                @click="tab = t.key"
              >
                <i :class="t.icon" class="w-4 text-center opacity-70"></i>
                <span class="grow text-left">{{ t.label }}</span>
                <span
                  v-if="tabGrade[t.key]"
                  class="badge badge-xs"
                  :class="gradeClass(tabGrade[t.key]!)"
                  :title="gradeLabel(tabGrade[t.key]!)"
                ></span>
              </button>
            </li>
          </ul>
        ...
        <div class="flex flex-col gap-6">
          ...
          <section v-show="tab === 'screen'" class="card bg-base-100 border border-base-300 shadow-sm">
            <div class="card-body">
              ...
              <p class="text-sm opacity-70">
                ...
                <em>cleared then rejected</em>: in hold mode that message reaches the reader.
                <em>Held then approved</em>
              ...
              <div class="stats stats-vertical sm:stats-horizontal bg-base-200 text-sm">
                ...
                <div class="stat py-2">
                  ...
                  <div
                    ...
                    :class="data.readout.screen.cleared_then_rejected ? 'text-error' : ''"
                  >
                ...
                <div class="stat py-2">
                  ...
                  <div class="stat-desc">
                    false positive · {{ data.readout.screen.held_then_rejected }} confirmed bad
                  </div>
                </div>
              </div>
              <p class="text-xs opacity-60">
          ...
          <section v-show="tab === 'parts'" class="card bg-base-100 border border-base-300 shadow-sm">
            <div class="card-body">
              ...
              <p class="text-sm opacity-70">
                ...
                is at or above the threshold (and beats the runner-up by the margin). The <em>floor</em> closes a
                listing as "no factory equivalent" when its best candidate is under the floor and the model also says
                none fit (≥ 0.70). Everything between the two is your queue. A floor close links nothing and is undone
                by Reopen on the correlations page; the count of those undos is the floor's error rate.
              </p>
              <div class="stats stats-vertical sm:stats-horizontal bg-base-200 text-sm">
                <div class="stat py-2">
          ...
          <section v-show="tab === 'mcp'" class="card bg-base-100 border border-base-300 shadow-sm">
            <div class="card-body">
              ...
              <p class="text-sm opacity-70">
                <strong>Measures:</strong> per table tool, how many near-miss lists got a pick, the average confidence
                and latency. A pick is a hint beside the list and cannot break an answer, so this is a cost-and-latency
          ...
          <section v-show="tab === 'queue'" class="card bg-base-100 border border-base-300 shadow-sm">
            <div class="card-body">
              ...
              <p class="text-sm opacity-70">
                ...
                clicked "attach" to the hinted row. <em>attached / with top</em> is the hit rate. Volume is tiny; this
                is a log.
              </p>
              <div class="stats stats-vertical sm:stats-horizontal bg-base-200 text-sm">
                <div class="stat py-2">
              ...
              <p class="text-sm opacity-70">
                ...
                P(safety-critical) is at or above 0.70, beside the seller's own flag. It only ever adds caution; check
                the badge on <NuxtLink to="/admin/models" class="link">3D Models</NuxtLink> for what it flagged.
              </p>
              <AdminTypesafeModeSwitch
                :row="mode('models')"
                :options="['off', 'on']"
          ...
          <section v-show="tab === 'review'" class="card bg-base-100 border border-base-300 shadow-sm">
            <div class="card-body">
              <h2 class="card-title text-lg">
                <i class="fas fa-clipboard-check text-secondary"></i> Submission review
              </h2>
              <p class="text-sm opacity-70">
                <strong>Measures:</strong> every submission gets one card (on-topic, prohibited, scam, quality,
                category, and a few checks per surface). <em>hint</em> shows the card on the admin item.
                <em>auto</em> approves a submission from a trusted member through the surface's own approval path when
                every check clears; anything else waits with the card saying why. Nothing is ever rejected
                automatically. The two numbers that decide <em>auto</em>: <strong>rejected despite clear</strong> (the
                card would have published something you refused) and <strong>approved despite flag</strong> (the card
                cries wolf).
              </p>
          ...
          <section v-show="tab === 'saved'" class="card bg-base-100 border border-base-300 shadow-sm">
            <div class="card-body">
              <h2 class="card-title text-lg">
                <i class="fas fa-bell text-primary"></i> Saved-search semantic fallback
              </h2>
              <p class="text-sm opacity-70">
                ...
                rule missed; the check is that they are wanted. Open two or three from
                <code>notification_queue</code> and read the listing against the saved text. Complaints (a saved "Cooper
                S" getting badge listings) mean the 0.7 threshold is too low.
              </p>
              <div class="stats stats-vertical sm:stats-horizontal bg-base-200 text-sm">
                <div class="stat py-2">
