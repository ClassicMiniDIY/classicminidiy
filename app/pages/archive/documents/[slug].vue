...
<template>
  <div class="min-h-screen bg-muted">
    ...
    <!--
      ...
      Every `key` here MUST be a real snake_case column on `archive_documents`
      AND appear in EDIT_TARGETS.document.columns in
      server/api/admin/queue/approve.post.ts — that handler maps
      `changes.{key}.to` straight onto the UPDATE with no camelCase translation,
      and rejects any key it doesn't recognise.
    -->
    <SuggestEditModal
      v-if="doc"
