...
- **External-model scraper (`server/utils/external-models/`):** Printables is fetched through its public GraphQL API (`printables.ts`) FIRST; its HTML pages have answered server fetches with a Cloudflare challenge since 2026-09, so the page + Microlink chain is only its fallback. Thingiverse still parses direct. Every other site goes to Microlink, which is on the free tier unless `NUXT_MICROLINK_API_KEY` is set on the Worker (it is not, as of 2026-09-17). Never route Printables back through the page scrape.
