...
- **Only moderation makes a listing `active`**, via `PUT /api/admin/listings/[id]/status` and `.../tier` (service-role). Client writes that publish are rejected server-side. `useListings().publishListing()` has no callers and must not be wired to a button. Any owner-facing status action must be exercised against a real non-admin session.
