...
- **Finds link previews (`server/api/exchange/external-listings/parse.post.ts`) identify as `ClassicMiniDIY-LinkPreview` to Facebook**, never as a browser: Facebook keys its Marketplace login wall on the User-Agent and serves the full OG head (title, description, image) to a link-preview UA. Every other source keeps the browser-shaped defaults. The price regex never scans Facebook HTML: the page embeds a similar-listings rail with other items' prices and not the listing's own. The parser must NEVER hard-fail on empty metadata; the manual-entry path is the fallback. Story: `docs/invariants/marketplace.md`.
