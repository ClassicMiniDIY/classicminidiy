...
- **`/search` answers in place; the palette hands off.** `SearchAskRow` with `inline` opens `SearchAnswerPanel` beside the results (one mount, moved between breakpoints with `order`; a second copy is a second request and a second message off the quota). The panel streams through `useChatAnswer`, the ONE transport definition `/chat` also uses — never construct a second `useChat` transport. "Continue in chat" must call `useChatHistory().load()` before `record()` (recording into an unloaded list overwrites every other conversation) and must not pass `?message=` (an auto-submit would send the question twice). Design: `docs/plans/2026-09-15-unified-search-phase-2.md`.
...
- **Wizard errors are per field and never shown on open.** A launch with a `kind` opens on step 2 with every required field empty, so `fieldErrors` is keyed by field and `shownError()` returns a message only after that field is left (`touched`) or after Continue/Submit is pressed (`attempted`, which also focuses the first `aria-invalid` control). Continue and Submit stay enabled so the press can reveal problems; the bottom list is a summary shown only after an attempt. A new required field needs a key in `fieldErrors` plus `v-bind="fieldAttrs(key)"` and its error span. `tests/unit/components/contribute/wizard-validation.test.ts`.
