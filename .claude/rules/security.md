...
- **`GET /api/chat/quota` is a peek, never a spend.** `peekChatQuota` reads the counters `consumeChatQuota` writes, mints no anonymous cookie, and reports `used: null` on any failure, which the search palette's Ask row treats as available; the chat route holds the real ceiling. It exists so the Ask row can change its offer at the moment of intent (sign in / membership / resets on the 1st). It is fetched once per palette open, after the input has focus, and `ChatWindow.vue` still has no status read in front of a chat load.
