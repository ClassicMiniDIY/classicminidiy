...
- `/admin/parts` counts come from the `admin_part_source_stats` view (three reads per load), never from per-source COUNTs through PostgREST: thirteen of those per source was ~480 round trips at 37 sources. A new figure on that screen is a column on the view in `classicminidiy-supabase` first, then `bun run gen:types`, then the route. A failed view read renders every count as null ("unknown"), never zero.
