...
- **Never export one name from two files in the same auto-import scope.** `shared/utils` is scanned alongside `app/utils`/`app/composables` on the client and alongside `server/utils/**` on Nitro, so a composable that re-exports a shared type (`export type { SearchResult }`) or redeclares a shared constant, or a `server/utils` file that re-exports its `shared/utils` twin, registers the name twice; unimport drops one and logs `Duplicated imports` per name. Import from `shared/` directly. `tests/static/auto-import-collisions.test.ts` enforces it.
