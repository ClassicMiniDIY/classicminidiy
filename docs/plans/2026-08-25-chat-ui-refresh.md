...
- **Sources render under the transcript at every breakpoint.** The permanent 320px rail and
  its empty placeholder are gone, and `UsefulLinksSidebar.vue` is deleted; `UsefulLinks.vue`
  is restyled onto daisyUI tokens. Fixes 19, and the desktop half of 5.
