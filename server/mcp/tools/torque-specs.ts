...
export default defineMcpTool({
  description:
    "Look up Classic Mini torque specifications. Every figure is in pound-feet, with a metric column derived from it; the `units` field of each response names the column exactly. Quote the figure in the unit `units` gives and never convert between them. Five sections: Engine (41 fasteners), Suspension (24), Clutch & Gearbox (23), Electrical (6) and General Purpose Fasteners (12). That last one is a fallback ready reckoner keyed by thread and spanner size for a fastener with no published spec of its own \u2014 always prefer a named fastener from the other sections when one matches, and say which you used. Search by fastener name (e.g. \"main bearing\", \"flywheel\", \"cylinder head\") or browse a whole section. MANY FIGURES ARE ENGINE-SPECIFIC: the same joint has different torques for 848/998 than for 970/1071/1275, and the rows are named differently ('bolts' vs 'nuts' vs 'set screws'). Include the displacement in the query when you know it, and always read each row's `notes` before quoting a figure.",
  ...
  inputSchema: {
    ...
    section: z
      ...
      .optional()
      .describe(
        'Restrict to one section. The only valid values are "Engine", "Suspension", "Clutch & Gearbox", "Electrical" and "General Purpose Fasteners" (or the keys engineTable, suspensionTable, gearboxTable, electricalTable, generalTable). Matched exactly and in full, so a bare Gearbox will not match the Clutch & Gearbox section.'
      ),
  ...
  async handler({ query, section, limit }, extra) {
    const data = (await getReferenceDataset<LookupData>('torque_specs')).value;
