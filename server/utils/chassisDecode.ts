...
/**
 * Chassis number decoding, shared by the REST route and the MCP tool.
 *
 * Extracted from server/api/decoders/chassis.ts, where it was module-private.
 * The MCP tool used to reach it by HTTP-PUTting the site's own route through
 * runtimeConfig.public.siteUrl — which meant a preview deploy decoded against
 * PRODUCTION, and every tool call re-entered the middleware stack as a mutating
 * /api/** request, spending the write rate limiter. That limiter keys on the
 * client IP, and serverless egress carries no x-real-ip, so those calls all
 * shared one global bucket and could 429 each other.
 *
 * This is a pure function of (chassisNumber, range): no event, no I/O, no H3.
 * Both callers import it directly.
 */
...
export interface ChassisPosition {
  position: number;
  value: string;
  name: string;
  matched: boolean;
...
export interface ChassisDecoderResponse {
  success: boolean;
  yearRange: string;
...
export const validateChassisNumber = (chassisNumber: string, range: ChassisRange): ChassisDecoderResponse => {
